...
//----------------------------------------------------------------------
int Thread::priorityToTime[PrLevel] = {1, 2, 4, 8};
...
Thread::Thread(char* threadName)
{
    ...
    stack = NULL;
    status = JUST_CREATED;
    priority_t = defaultPr;
    timeSlice_t = priorityToTime[priority_t];
    ptime = 0;
#ifdef USER_PROGRAM
    space = NULL;
#endif
    ...
    this->threadID = useableTid();
    this->userID = getuid();
    (void) interrupt->SetLevel(oldLevel);
}

/*Thread::Thread(threadname, priority)*/
...
Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    if(this->threadID == -1){
        DEBUG('t', "Number of Threads over 128!\n");
    //printf("Number of Threads over 128!\n");
        delete this;
        return;
    }
    else{
...
/*judge if *this is a new thread*/
bool 
Thread::isNewThread()
{ return machineState[PCState] == (int*)ThreadRoot;}

/*make system to Tick 50 times*/
void 
Thread::threadTick(){
    const int repeat = 3;
...
Thread::regainTime(){
    ...
    timeSlice_t = Thread::priorityToTime[priority_t];
}

void
Thread::syncPtime(){
    ptime = stats->totalTicks;
}
